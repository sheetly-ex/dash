...
const TopNavItem: React.FC<TopNavItemProps> = ({ icon, label, active, onClick }) => {
  return (
    ...
    >
      ...
      <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 group-active:scale-95 shadow-xs border ${active
        ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-500/20'
        : 'bg-surface-muted text-app-muted border-app-muted hover-surface hover:text-app'
        }`}>
        {icon}
      </div>
